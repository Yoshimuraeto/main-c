...
class MainR:
    ...
    def forward(self):
        ...
        if st.session_state.count >= 5:
            system_input = "日記からわかるこの人の行動の特徴は何か？"
            ...
            with open("text/questions.txt", "r", encoding="utf-8") as f:
                questions = f.read()
            system_input = f"以下の質問に回答のみを対話した人になりきり数字で答えて下さい。ただしその際にこの人の行動の特徴を考慮してください。{questions}. 回答は以下のフォーマットにしてください。「1,2,3,4,5,…」"
            assistant_response = self.generate_and_store_response(
                system_input, st.session_state.db
            )
            st.markdown(
            ...
            st.markdown(
                f'これで今回の会話は終了です。こちらをクリックして結果を確認してください: <a href="{group_url_with_id}" target="_blank">リンク</a>',
                unsafe_allow_html=True,
            )
            self.disable_chat_input()
